...
#ifdef USE_X11
#include "../libbitblit/x11-backend/bitx11.h"
#endif
/*}}}  */
/*{{{  #defines*/
...
/*{{{  init_all*/
void init_all(register BITMAP *where)
{
  maxv = BIT_HIGH(where);
  hmaxv = maxv>>1;
  maxh = BIT_WIDE(where);
...
static void cordic(short *x, short *y, register short scale, register short count)
{
...
/*}}}  */
/*{{{  xplot*/
static int xplot(register BITMAP *where, register int x, register int y, register int col, int state)
{
...
/*}}}  */
/*{{{  project*/
static int project(register BITMAP *where, register short x, register short y, register short z, register int col, register short state)
{
...
/*}}}  */
/*{{{  dofly*/
static void dofly (BITMAP *where)
{
  register short i;
  register struct st *stp;
...
void copyright(BITMAP *where, char *password)
{
  ...
  for(;;)
  {
    ...
    if( sel > 0) {
	  ...
	  if (ev.type == KeyPress)
	    XLookupString(&ev.xkey, readp, 1, NULL, NULL);
	  else
	    continue;
#else
